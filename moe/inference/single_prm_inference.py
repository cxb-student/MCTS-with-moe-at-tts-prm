...
def single_prm_inference(q):
    ...
    # 2. 定义Qwen PRM奖励模型
    class PRM(RewardModelCallingFunction):
        def __init__(self):
            # 初始化RewardModelBaseConfig对象
            config = RewardModelBaseConfig()
            config.prm_step_tag = "<extra_0>"
            config.format_str = "{question}\n{answer}"
            config.rm_serve_type = "qwen"
    ...
    cot_env = CoTEnv(
        ...
        problem_format_str="You are a helpful assistant. A conversation between User and Assistant. The user asks a question, and the Assistant solves it. The Assistant first thinks about the reasoning process in the mind and then provides the user with the answer.\
